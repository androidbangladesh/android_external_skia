...
mychar mytext[1024];
    ...
    while (iter.next()) {
        if (mybyteLength > 0)  {
        iter.fDevice->drawText(iter, (const void*)mytext, mybyteLength, x, y, paint);
        } else {
...
void SkCanvas::drawPosText(const void* text, size_t byteLength,
   ...
   mychar mytext[1024];
    ...
    while (iter.next()) {
        if (mybyteLength > 0) 
        iter.fDevice->drawPosText(iter, (const void*)mytext, mybyteLength, &pos->fX, 0, 2,
                                  paint);
        else
...
void SkCanvas::drawPosTextH(const void* text, size_t byteLength,
                            ...
                            const SkPaint& paint) {
    ITER_BEGIN(paint, SkDrawFilter::kText_Type)
  //  	__android_log_print(ANDROID_LOG_DEBUG  , "chain","%s","SkCanvas::drawPOSTextH");
    while (iter.next()) {
        iter.fDevice->drawPosText(iter, text, byteLength, xpos, constY, 1,paint);
    }
...
void SkCanvas::drawTextOnPath(const void* text, size_t byteLength,
                              ...
                              const SkPaint& paint) {
    ITER_BEGIN(paint, SkDrawFilter::kText_Type)
 //   	__android_log_print(ANDROID_LOG_DEBUG  , "chain","%s","SkCanvas::drawTextOnPath");
    while (iter.next()) {
        iter.fDevice->drawTextOnPath(iter, text, byteLength, path,
                                     matrix, paint);
//	__android_log_print(ANDROID_LOG_DEBUG  , "drawtext","%s %s","drawtextONPATH", text);
    }
...
void SkCanvas::drawData(const void* data, size_t length) {
  ...
  //  	__android_log_print(ANDROID_LOG_DEBUG  , "chain","%s","SkCanvas::drawData");
}
...
void SkCanvas::drawTextOnPathHV(const void* text, size_t byteLength,
    ...
    SkMatrix    matrix;
//        	__android_log_print(ANDROID_LOG_DEBUG  , "chain","%s","SkCanvas::drawTextOnPathHV");
